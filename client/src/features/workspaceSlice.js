...
export const fetchWorkspaces = createAsyncThunk("workspace/fetchWorkspaces", async ({getToken}) => {
    try{
        ...
        const {data} = await axiosInstance.get("/workspaces", { headers: {Authorization: `Bearer ${await  getToken()}`}  });
...
const workspaceSlice = createSlice({
    ...
    // Define extraReducers for async actions if needed
    extraReducers: (builder) => {
        ...
        builder.addCase("fetchWorkspaces/fulfilled", (state, action) => {
            state.loading = false;
            state.workspaces = action.payload;
            //Set current workspace to the first workspace if not already set
            if(action.payload.length > 0){
                const currentWorkspaceId = localStorage.getItem("currentWorkspaceId");
                const workspaceToSet = action.payload.find(w => w.id === currentWorkspaceId) || action.payload[0];
